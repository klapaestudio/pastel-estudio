...
// facturación y mensajes. No tocar en producción — pensado para desarrollo/demo.
// Correr con: npm run seed:demo (requiere haber corrido "npm run seed" antes).

import bcrypt from "bcryptjs";
import { prisma } from "../src/lib/prisma";
import { addDays, addMonths } from "../src/lib/followup";
...
async function main() {
  ...
  // ── Usuarios de staff (uno por rol, además del admin del seed base) ──────
  for (const u of [
  ...
  const provFlete = await prisma.proveedor.create({
    data: {
      nombre: "Flete Expreso Rosario",
      tipo: "TRANSPORTE",
      ciudad: "Rosario",
      telefono: "341-555-0288",
      contacto: "Marcos Peralta",
    },
  });

  const linoNatural = await prisma.material.create({
    data: {
      proveedorId: provTelas.id,
      nombre: "Lino natural crudo",
      unidad: "m",
      costoActualizado: 4200,
      stockEstado: "HAY_STOCK",
      stockCantidad: 85,
    },
  });
  const panaGruesa = await prisma.material.create({
    data: {
      proveedorId: provTelas.id,
      nombre: "Pana gruesa mostaza",
      unidad: "m",
      costoActualizado: 5100,
      stockEstado: "QUEDANDO_SIN_STOCK",
      stockCantidad: 6,
    },
  });
  await prisma.material.create({
    data: {
      proveedorId: provTelas.id,
      nombre: "Terciopelo premium verde botella",
      unidad: "m",
      costoActualizado: 7800,
      stockEstado: "SIN_STOCK",
      stockCantidad: 0,
    },
  });

  await prisma.manoObraTarifa.create({
    data: { proveedorId: provCarpinteria.id, tipoTrabajo: "Armado de estructura madera", costo: 18000 },
  });
  await prisma.manoObraTarifa.create({
    data: { proveedorId: provCarpinteria.id, tipoTrabajo: "Tapicería completa", costo: 22000 },
  });

  await prisma.pagoProveedor.create({
    data: {
      proveedorId: provTelas.id,
      concepto: "Compra de tela — pedido #114",
      monto: 63000,
      fechaVencimiento: addDays(hoy, 5),
      pagada: false,
    },
  });
  await prisma.pagoProveedor.create({
    data: {
      proveedorId: provCarpinteria.id,
      concepto: "Estructuras sillón Munich x4",
      monto: 72000,
      fechaVencimiento: addDays(hoy, 12),
      pagada: false,
    },
  });
  await prisma.pagoProveedor.create({
    data: {
      proveedorId: provFlete.id,
      concepto: "Flete entrega Bianchi",
      monto: 9500,
      fechaVencimiento: addDays(hoy, -10),
      pagada: true,
      fechaPago: addDays(hoy, -9),
    },
  });

  // ── Productos estándar (Colección) ───────────────────────────────────────
  const sillonMunich = await prisma.productoEstandar.create({
    data: {
      nombre: "Sillón Munich",
      descripcion: "Sillón de una plaza, estructura de madera maciza tapizada.",
      medidas: "90x85x75cm",
  ...
  await prisma.productoEstandarMaterial.create({
    data: { productoEstandarId: sillonMunich.id, materialId: linoNatural.id, cantidad: 3.2 },
  });
  await prisma.stockProducto.create({
    data: { productoEstandarId: sillonMunich.id, cantidad: 4, tela: "Lino natural crudo", embalaje: "SIMPLE" },
  });

  const sillaCopenhague = await prisma.productoEstandar.create({
    data: {
      nombre: "Silla Copenhague",
      descripcion: "Silla tapizada de líneas nórdicas.",
      medidas: "45x50x82cm",
      manoObraEstructura: 6000,
      manoObraTapiceria: 4500,
      precioVenta: 68000,
    },
  });
  await prisma.productoEstandarMaterial.create({
    data: { productoEstandarId: sillaCopenhague.id, materialId: panaGruesa.id, cantidad: 1.1 },
  });
  await prisma.stockProducto.create({
    data: {
      productoEstandarId: sillaCopenhague.id,
      cantidad: 0,
      tela: "Pana gruesa mostaza",
      embalaje: "DOBLE",
      mensajePredefinido: "Sin stock — próximo lote en 3 semanas.",
    },
  });

  // ── Mensajes prearmados (categorías ya creadas por el seed base) ────────
  ...
  if (catStock) {
    await prisma.mensaje.create({
      data: {
        categoriaId: catStock.id,
        titulo: "Hay stock",
        texto: "¡Hola {{nombre}}! Sí, tenemos stock de {{producto}} en tela {{tela}}. ¿Querés que te pase el precio?",
      },
    });
  }
  if (catFollowUp) {
    await prisma.mensaje.create({
      data: {
        categoriaId: catFollowUp.id,
        titulo: "Recordatorio presupuesto enviado",
        texto: "Hola {{nombre}}, ¿pudiste ver el presupuesto que te enviamos el {{fecha}}? Quedo atenta ante cualquier duda.",
      },
    });
  }
  if (catEnvio) {
    await prisma.mensaje.create({
      data: {
        categoriaId: catEnvio.id,
        titulo: "Pedido despachado",
        texto: "Hola {{nombre}}, tu pedido de {{producto}} ya fue despachado. ¡Te avisamos apenas llegue!",
      },
    });
  }

  // ── PROSPECTOS (distintos perfiles / orígenes / estados) ────────────────

  const p1 = await prisma.contacto.create({
    data: {
      etapa: "PROSPECTO",
  ...
  const p2 = await prisma.contacto.create({
    data: {
      etapa: "PROSPECTO",
  ...
  const p3 = await prisma.contacto.create({
    data: {
      etapa: "PROSPECTO",
      nombre: "Sofía Ibarra",
  ...
  const p4 = await prisma.contacto.create({
    data: {
      etapa: "PROSPECTO",
  ...
  const presuBianchi = await prisma.presupuesto.create({
    data: {
  ...
  const cobroBianchi = await prisma.cobro.create({
    data: {
      contactoId: cliBianchi.id,
      concepto: "Sillón Munich tapizado",
      categoria: "COLECCION",
      gananciaTotal: 108000,
    },
  });
  await prisma.cuota.create({
    data: {
      cobroId: cobroBianchi.id,
      numero: 1,
      fecha: addDays(hoy, -68),
      monto: 77250,
      costos: 34000,
      gastos: 9500,
      pagada: true,
      comprobanteEmitido: true,
      comprobanteTipo: "Factura B",
      comprobantePuntoVenta: "0001",
  ...
  await prisma.archivoProyecto.create({
    data: { contactoId: cliOtero.id, nombre: "Plano general PB.pdf", tipo: "plano" },
  });
  await prisma.archivoProyecto.create({
    data: { contactoId: cliOtero.id, nombre: "Memoria descriptiva.docx", tipo: "documento" },
  });
  ...
  await prisma.cuota.create({
    data: {
      cobroId: cobroSuarez.id,
      numero: 1,
      fecha: addDays(hoy, -20),
      monto: 87000,
      costos: 43000,
      gastos: 0,
      pagada: true,
  ...
  await prisma.tareaAgenda.create({
    data: {
      ...
      fechaFin: addDays(hoy, 2),
      ubicacion: "Taller Pastel Studio",
      notas: "Revisar pedidos de la semana y stock crítico.",
    },
  });
  await prisma.tareaAgenda.create({
    data: {
      titulo: "Medición en obra — local Otero",
      tipo: "CLIENTE_RESERVABLE",
      contactoId: cliOtero.id,
      fechaInicio: addDays(hoy, 5),
      fechaFin: addDays(hoy, 5),
      ubicacion: "Av. Pellegrini 1450, Rosario",
    },
  });
  await prisma.tareaAgenda.create({
    data: {
  ...
  await prisma.franjaBloqueada.create({
    data: { fecha: addDays(hoy, 1), horaInicio: "13:00", horaFin: "14:00", motivo: "Almuerzo equipo" },
  });

  // ── Facturación: gastos generales y retiros de socias ───────────────────
  await prisma.gastoGeneral.create({
    data: { concepto: "Alquiler taller", categoria: "Fijo", monto: 350000, fecha: addDays(hoy, -5) },
  });
  await prisma.gastoGeneral.create({
    data: { concepto: "Insumos de librería y embalaje", categoria: "Variable", monto: 18500, fecha: addDays(hoy, -2) },
  });
  ...
  console.log("Prospectos: Julieta Fernández (particular), Martín Larrain (arquitecto), Sofía Ibarra (diseñadora), Nicolás Roldán (rechazado), Carla Méndez (postergado).");
  console.log("Clientes: Lucía Bianchi (colección), Fernando Otero (arquitectura, con portal), Valentina Suárez (personalizado).");
  console.log("Login portal cliente: fernando.otero@example.com / pastel2026");
  console.log("Login staff: ventas@pastelstudio.com, taller@pastelstudio.com, finanzas@pastelstudio.com — todos con pastel2026");
}
